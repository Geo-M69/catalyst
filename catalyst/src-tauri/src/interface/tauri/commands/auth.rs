...
#[tauri::command]
pub(crate) fn logout(state: State<'_, AppState>) -> Result<(), String> {
...
#[tauri::command]
pub(crate) fn get_session(state: State<'_, AppState>) -> Result<Option<PublicUser>, String> {
    let connection = open_connection(&state.db_path)?;
    cleanup_expired_sessions(&connection)?;
...
#[tauri::command]
pub(crate) async fn start_steam_auth(state: State<'_, AppState>) -> Result<SteamAuthResponse, String> {

    ...
    if let Some(data) = maybe_data {
        if let Some(categories) = data.get("categories").and_then(Value::as_array) {
            let canonical_from_desc = |desc: &str| -> Option<(String, String)> {
                ...
                if lowered.contains("trading card")
                    || lowered.contains("trading cards")
                    || lowered.contains("trading-cards")
                {
        ...
        if controller_level.is_none() {
            if as_string.contains("full controller")
                || as_string.contains("full controller support")
            {
                controller_level = Some("Full".to_string());
                seen_keys.insert("controller-support".to_string());
            } else if as_string.contains("partial controller")
                || as_string.contains("partial controller support")
            {
                controller_level = Some("Partial".to_string());
                seen_keys.insert("controller-support".to_string());
            }
        ...
        if (as_string.contains("workshop") || as_string.contains("steam workshop"))
            && !as_string.contains("trading card")
            && !as_string.contains("trading cards")
        {
    ...
    if let Some(ctrl_val) = controller_level {
        if seen_keys.insert("controller-support".to_string()) {
            let label = match ctrl_val.to_ascii_lowercase().as_str() {
                "full" | "full controller" | "full controller support" => {
                    "Full Controller Support".to_string()
                }
                "partial" | "partial controller" | "partial controller support" => {
                    "Partial Controller Support".to_string()
                }
